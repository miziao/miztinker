...
public class HadStomachHandler {
    ...
    public static void onFoodEatFinish(LivingEntityUseItemEvent.Finish event) {
        ...
        for (EquipmentSlot slot : ARMOR_SLOTS) {
            ...
            if (!stack.isEmpty() && stack.is(slimeknights.tconstruct.common.TinkerTags.Items.MODIFIABLE)) {
                try {
                    ...
                    if (!tool.isBroken()) {
                        level = Math.max(level, tool.getModifierLevel(MiztinkerModifiers.HAD_STOMACH_STATIC_MODIFIER.get()));
                    }
        ...
        if (level > 0) {
            MiztinkerModifiers.HAD_STOMACH_STATIC_MODIFIER.get().healByFood(player, itemStack);
        }
    }
}