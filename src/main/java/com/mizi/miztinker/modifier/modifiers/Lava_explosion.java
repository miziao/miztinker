...
public class Lava_explosion extends NoLevelsModifier implements InventoryTickModifierHook {
    ...
    public static void explode(Player player) {
        ...
        for (LivingEntity entity : entities) {
            entity.hurt(level.damageSources().explosion(player, player), 3500.0f);
        }
    }
    ...
    /** 清除 3x3x3 范围的水 */
    private void clearLava(Level world, BlockPos center) {
        BlockPos.betweenClosedStream(center.offset(-1, -1, -1), center.offset(1, 1, 1))
                .forEach(pos -> {
                    BlockState state = world.getBlockState(pos);
                    if (state.getBlock() == Blocks.LAVA || state.getFluidState().is(Fluids.WATER)) {
